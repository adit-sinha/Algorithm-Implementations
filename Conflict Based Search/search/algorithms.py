...
class CBSState:
    ...
    def is_solution(self):
        ...
        for i in range(self._k):
            ...
            for j in range(i + 1, self._k): 
                p_j = self._paths[j] 
                for t in range(len(p_i)):
                    if t >= len(p_j):  #checking if within bounds
                        break 
                    if p_i[t] == p_j[t]:  
                            return False, (State(p_j[t].get_x(), p_j[t].get_y()), t)
        return True, None
    ...
    def successors(self):
        ...
        for i in range(self._k):
            ...
            for j in range(i + 1, self._k):
                p_j = self._paths[j]
                if (t < len(p_i) and t < len(p_j)) and (p_i[t] == p_j[t]):
                    a1 = i
                    a2 = j
                    break
        ...
        c1.set_constraint(s, t, a1)
        c2.set_constraint(s, t, a2)
    ...
    def get_paths(self):
        """
        Returns the paths for all agents
        """
        return self._paths
...
class CBS():
    def search(self, start):
        ...
        """
        scost = start.compute_cost()

        open = []
        if (scost == float('inf')): #based on pseudocode
        ...
        while len(open) > 0:
            ...
            is_sol, _ = n.is_solution() 
            if is_sol:
                return n.get_paths(), n.get_cost()  

            clist = n.successors()
            for c in clist:
                c.compute_cost()
                if c.get_cost() != float('inf'): #based on pseudocode
